    ...
    <>
      ...
      <header className="sticky top-0 z-40 border-b border-[var(--chrome-border)] bg-[var(--chrome-bg)]">
        <div className="mx-auto flex h-[4.75rem] max-w-[80rem] items-center justify-between gap-6 px-5 sm:px-8">
          <Link href="#top" className="flex shrink-0 items-center" aria-label="Sanatec Marketing, home">
            <Image
              src="/images/logo.png"
          ...
          <nav aria-label="Primary" className="hidden items-center gap-1 lg:flex">
            <Link
              href="#top"
              className="relative px-3.5 py-2.5 text-sm font-medium text-[var(--chrome-accent)] after:absolute after:bottom-0 after:left-3.5 after:h-0.5 after:w-[calc(100%-1.75rem)] after:rounded-full after:bg-[var(--chrome-accent)] after:content-['']"
            >
              Home
            </Link>
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-50 bg-[var(--chrome-bg)] lg:hidden">
          ...
          <div className="h-[calc(100%-4.75rem)] overflow-y-auto px-5 py-6 sm:px-8">
            {["Home", "Products", "Brands", "Resources", "About Us", "Contact Us"].map(
              (label) => (
                <a
                  key={label}
                  href={label === "Products" ? "#categories" : "#top"}
                  onClick={() => setMobileOpen(false)}
